...
async function main() {
    ...
    if (!options.output || !options.saf) {
        ...
        let resolver = new Resolver({ outputPath: options.output, scopePath: options.saf, directoryPath: options.directory, vsn: options.defaultversion, configPath: options.config, interpreterType: options.interpreter, converterType: options.converter });
