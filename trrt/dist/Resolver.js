...
const download = __require("download");
...
export class Resolver {
    ...
    // todo switch scope
    scope;
    tmpLocalMrgFile = "docs\\tev2\\glossaries\\mrg.mrgtest.yaml"; // temp
    mrgWritePath = "./mrg.yaml";
    config;
    directory = ".";
    ...
    processConfig() {
        ...
        const config = new Map(Object.entries(yaml.load(fs.readFileSync(this.config, 'utf8'))));
    ...
    setOptionalParams(directoryPath, vsn, interpreterType, converterType) {
        if (directoryPath) {
    ...
    getMrgUrl() {
        ...
        const safDocument = new Map(Object.entries(yaml.load(fs.readFileSync(this.scope, 'utf8'))));
        // JSON.stringfy() used to force object to string casting as javascript does not support typing otherwise
        const scopeMap = new Map(Object.entries(yaml.load(JSON.stringify(safDocument.get("scope")))));
        var mrgURL = "";
    ...
    async readGlossary() {
        ...
        if (this.tmpLocalMrgFile) {
            ...
            const mrgDocument = yaml.load(fs.readFileSync(this.tmpLocalMrgFile, 'utf8'));
            ...
            if (mrgURL != "") {
                ...
                const mrgDocument = yaml.load(fs.readFileSync(this.mrgWritePath, 'utf8'));
    ...
    populateGlossary(mrgDocument, glossary) {
        const mrg = new Map(Object.entries(mrgDocument));
        for (const [key, value] of Object.entries(mrg.get("entries"))) {
            var alternatives;
            const innerValues = new Map(Object.entries(yaml.load(JSON.stringify(value))));
    ...
    interpertAndConvert(data, glossary) {
        const matches = data.matchAll(this.interpreter.getGlobalTermRegex());
